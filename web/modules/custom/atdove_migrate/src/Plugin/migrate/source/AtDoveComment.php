 ...
 *
 * @MigrateSource(
 *   id = "atdove_d7_comment",
 *   source_module = "comment"
 * )
 */
class AtDoveComment extends FieldableEntity {
  ...
  public function prepareRow(Row $row) {
    ...
    $comment_type = $row->getSourceProperty('comment_type');
    $node_type = $row->getSourceProperty('node_type');
 
    $body_query = $this->select('field_data_comment_body', 'cb')
      ->fields('cb', ['comment_body_value'])
      ->condition('entity_id', $cid)
      ->execute()
      ->fetchField();

    $row->setSourceProperty('comment_body', $body_query);
    $row->setSourceProperty('comment_type', 'comment_node_' . $node_type);
    $comment_field_name =  'field_' . $node_type . '_comments';
    $row->setSourceProperty('comment_field_name', $comment_field_name);
    $row->setSourceProperty('bundle', 'atdove_' . $node_type);
    ...
    if ($node_type == 'article' || $node_type == 'video') {
      $row->setSourceProperty('entity_id', $nid);
    }
